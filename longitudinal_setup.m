...
function main()  
    ...
    %   addt: [hdotc]
        ...
        [uct,~,dxdt_c] = LongControlOutSimple(t,x,y_way,addt,AC);
        dxdt = LonDynNoLin(t,x,uct,AC);
        addt(1) = t;
        x_add = storefun(addt);
        %out_stored = out_store();
        %out_store = storefun([out_stored;[t,uout(:)',uct(:)']]); % CREARE UN VETTORE DI OUTPUT DA SALVARE
        dxdt = [dxdt(:);dxdt_c(:)];
    end
   ...
   %'Events',@events);
    ...
    switch CHS
        case 'ctrl'
            ...
            [t,x] = ode45( @(t,x)LongDynNLConSimp( t,x,AC,y_way ),...
                                [te,Tfin],ye,options );
            x_aux = out_store(); x_aux = x_aux(2:end,:);
            plot_results(CHS,t,x,x_aux);
        case 'waypoint'
            te = 0;                                              % Starting time
